...
template<> struct StateTraits<State::Param8>            {using type = signed char;};
template<> struct StateTraits<State::ParamU8>           {using type = unsigned char;};
template<> struct StateTraits<State::Param16>           {using type = short;};
template<> struct StateTraits<State::ParamU16>          {using type = unsigned short;};
template<> struct StateTraits<State::Param32>           {using type = int;};
template<> struct StateTraits<State::ParamU32>          {using type = unsigned int;};
...
template<> struct StateTraits<State::Param64>           {using type = long long;};
template<> struct StateTraits<State::ParamU64>          {using type = unsigned long long;};
...
public:
    ...
    void decodeParam(time)
    {
        if (sizeof(Logger::TagType) + sizeof(uint64_t) == mReadSz)
        {
            ...
            if (!mLogTimeBaseSet)
            {
                mLogTimeBaseSet = true;
                mLogTimeBase = mLogTime;
            }
            mLogTime -= mLogTimeBase;
            mState = State::Thread;
            mReadSz = 0;
    ...
    void decodeParam(thread)
    {
        if (sizeof(Logger::TagType) + sizeof(uint64_t) == mReadSz)
        {
            ...
            mSs << std::dec << double(mLogTime)/1000000 << " s ";
...
private:
    ...
    bool mLogTimeBaseSet = 0;
    uint64_t mLogTimeBase = 0;
    uint64_t mLogTime;
    uint64_t mLogThread;
